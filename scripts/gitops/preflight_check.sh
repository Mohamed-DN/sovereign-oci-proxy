...
while IFS= read -r yfile; do
  [ -z "$yfile" ] && continue
  YAML_FILES+=("$yfile")
done < <(find "${PROJECT_ROOT}" \( -name "*.yaml" -o -name "*.yml" \) ! -path "*/.agents/*" ! -path "*/.git/*" ! -path "*/templates/*")
...
for pattern in "${SUSPECT_PATTERNS[@]}"; do
  grep_out=$(grep -rEn --exclude-dir=".git" --exclude-dir=".agents" --exclude="preflight_check.sh" "${pattern}" "${PROJECT_ROOT}" 2>/dev/null || true)
  if [ -n "${grep_out}" ]; then
