...
func main() {
	...
	log.Printf("[SOVEREIGN-RELAY] Starting camouflaged DERP-v4 relay [Region: %s, TCP: %s, UDP STUN: %s]...", *region, *listenAddr, *stunAddr)
	...
	log.Printf("[SOVEREIGN-RELAY] DERP-v4 Relay operational with WebSocket camouflage at /ws/v4/relay")

	sigChan := make(chan os.Signal, 1)
	signal.Notify(sigChan, os.Interrupt, syscall.SIGTERM)
	<-sigChan

	log.Println("[SOVEREIGN-RELAY] Draining relay connections and shutting down...")
	_ = server.Close()
	fmt.Println("Sovereign relay stopped cleanly.")
}
